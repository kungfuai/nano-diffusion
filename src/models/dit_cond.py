...
class TimestepEmbedder(nn.Module):
    ...
    def forward(self, t):
        ...
        t_freq = t_freq.to(
            self.mlp[0].weight.device
        )  # Ensure t_freq is on the same device as the model
...
class TextEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.tokenizer = CLIPProcessor.from_pretrained(model_name)
        self.hidden_dim = hidden_dim
        self.projector = nn.Linear(text_feature_dim, hidden_dim)

        # a null embedding used when a token is dropped
        self.null_embedding = nn.Parameter(torch.zeros(1, hidden_dim))
        self.dropout_prob = dropout_prob

    def forward(self, text: str, train: bool = True, force_drop_ids=None):
        # Process text
        ...
        # Apply token dropout if training
        if train or force_drop_ids is not None:
            text_features = self.token_drop(text_features, force_drop_ids)
    ...
    def token_drop(self, embeddings, force_drop_ids=None):
        ...
        # Replace dropped embeddings with cfg_embedding
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.y_embedder = TextEncoder(
            model_name="zer0int/CLIP-GmP-ViT-L-14", hidden_dim=hidden_size
        )
